...
export function extractMemoryFromMsg(msg, mood) {
  ...
  // Study/NEET context
  if (/neet|jee|medical|entrance|biology|physics|chemistry/.test(m)) updates.isStudent = true;
  if (/neet/.test(m)) updates.exam = 'NEET 2026';
...
export function buildAriaContext({ userMsg, mood, intent, memory, lastReply, attachment }) {
  ...
  if (memory?.exam)        parts.push(`[HE IS PREPARING: ${memory.exam}]`);
  if (memory?.isStudent)   parts.push(`[HE IS A STUDENT]`);
