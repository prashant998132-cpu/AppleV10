...
export async function saveDailyLog(userId, log) {
  ...
  // Update existing log for today instead of creating duplicate
  const existingIdx = logs.findIndex(l => (l.date || l.log_date) === today);
  const newLog = { ...log, date: today, log_date: today, updatedAt: Date.now() };
  if (existingIdx >= 0) {
...
export async function exportAllData(userId) {
  try {
    const [profile, goals, memories, conversations, logs, habits, knowledge] = await Promise.all([
      getProfile(userId),
      getGoals(userId),
      getMemories(userId, { limit: 500 }),
      getConversations(userId, 100),
      getDailyLogs(userId, 90),
      getHabits(userId),
      getKnowledge(userId, 100),
    ]);
    return {
      exportedAt: new Date().toISOString(),
      version: '12.0',
      profile, goals, memories, conversations, logs, habits, knowledge,
    };
  } catch {
    return { profile: await getProfile(userId), exportedAt: new Date().toISOString() };
  }
}
