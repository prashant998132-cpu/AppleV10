...
export async function executeCommand(parsed, { setTheme, addMsg, navigate } = {}) {
  ...
  switch (parsed.type) {
    ...
    case 'neet_schedule': {
      ...
      const days2 = Math.max(0,Math.round((new Date('2026-05-03')-new Date())/86400000));
      ...
      const schedText = 'NEET 2026 — ' + days2 + ' din baaki!\n\n' + lines.join('\n') + (next2 ? '\n\nNext: ' + next2.label + ' at ' + next2.time : '');
      return { success: true, message: schedText, type: 'neet_schedule', handled: true, response: schedText };
    }
      ...
      return {
        handled: true,
        response: `📋 **Tera Daily Routine — NEET 2026**\n\n${routineText}\n\n---\n✅ Sab alarms already set hain tere phone pe! Bas follow karna hai.`
      };
    }
...
🔍 **Search:**
"Google karo NEET syllabus" / "YouTube search karo Biology lectures"
