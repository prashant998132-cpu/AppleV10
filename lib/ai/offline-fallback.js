...
const RESPONSES = {
  ...
  time: [
    `Abhi ${new Date().toLocaleString('en-IN', { timeZone:'Asia/Kolkata', hour:'2-digit', minute:'2-digit' })} ho raha hai!`,
    `Time: ${new Date().toLocaleString('en-IN', { timeZone:'Asia/Kolkata', hour:'2-digit', minute:'2-digit' })} — ${new Date().toLocaleDateString('en-IN', { timeZone:'Asia/Kolkata', weekday:'long' })}`,
  ],
  date: [
  ...
  neet: [
    '📚 NEET 2026 — 3 May hai target! Abhi network nahi hai, lekin concept yaad karo:\n\n🧬 Biology: NCERT line by line — diagrams sahi se banao\n⚛️ Physics: Numericals daily practice karo\n🧪 Chemistry: Reactions aur equations revise karo',
    '🎯 NEET ke liye offline tips:\n• Biology = 360 marks — sabse important\n• Physics numericals daily\n• Chemistry NCERT reactions ratto\n• Mock tests = real exam feel',
    '📖 Abhi AI se connect nahi ho pa raha. Lekin ek kaam karo — NCERT ka ek chapter kholo aur diagramatic notes banao. Best use of this time!',
  ],
  biology: [
    '🧬 Biology offline tips:\n• Cell division — Mitosis vs Meiosis diagram banao\n• Photosynthesis + Respiration equations yaad karo\n• Human Physiology — digestive/circulatory/nervous system\n• Genetics — Mendel ke laws aur exceptions (Incomplete dominance, Codominance)\n• NCERT diagrams sahi se label karo',
    '🔬 NEET Biology ke liye: NCERT sabse important hai. Har chapter ke important points sticky notes pe likhlo. Network aane pe MCQ practice karenge!',
  ],
  physics_neet: [
    '⚛️ NEET Physics offline tips:\n• Laws of Motion — FBD practice karo\n• Work-Energy theorem numerical\n• Current Electricity — Kirchhoff laws\n• Optics — lens/mirror formula\n• Modern Physics — photoelectric effect',
    '📐 Physics formula revision karo offline:\nv = u + at | s = ut + ½at² | F = ma | E = mc² | P = IV',
  ],
  chemistry_neet: [
    '🧪 NEET Chemistry offline:\n• Organic — reaction mechanisms\n• Inorganic — periodic properties\n• Physical — numericals practice karo\n• Important: Aldehydes, Ketones reactions yaad karo',
    '⚗️ Chemistry ke liye reactions likhte raho notebook mein. Yeh offline bhi ho sakta hai aur bahut effective hai!',
  ],
  offline_notice: [
...
export function offlineFallback(message) {
  ...
  // NEET specific
  if (/\b(neet|jee|medical|entrance|3 may|may 3)\b/.test(m))
    return pick(RESPONSES.neet);
  if (/\b(biology|bio|genetics|cell|photosynthesis|human body|ncert bio)\b/.test(m))
    return pick(RESPONSES.biology);
  if (/\b(physics|numerical|mechanics|optics|current|electricity|motion)\b/.test(m))
    return pick(RESPONSES.physics_neet);
  if (/\b(chemistry|organic|inorganic|reaction|periodic|mole|molarity)\b/.test(m))
    return pick(RESPONSES.chemistry_neet);
