...
export function generateFollowUps(userMessage, aiReply, mode = 'auto') {
  ...
  if (/neet|jee|medical|entrance/.test(lower)) {
    return ['📚 Aaj ka study plan do', '❓ MCQ practice karo', '🧬 Biology important topics', '📊 NEET countdown check karo'];
  }
  if (/biology|bio|genetics|cell|photosynthesis/.test(lower)) {
    return ['🔬 Diagram banao saath mein', '❓ MCQ do is topic pe', '📝 Short notes chahiye', '🧠 Mnemonics batao'];
  }
  if (/physics|numerical|mechanics|optics/.test(lower)) {
    return ['📐 Aur numericals do', '📝 Formula sheet chahiye', '🔁 Concept dobara explain karo', '⚡ Shortcut tricks batao'];
  }
  if (/chemistry|organic|reaction|periodic/.test(lower)) {
    return ['⚗️ Reactions list do', '❓ MCQ practice', '📝 Notes banao', '🔁 Revision karo'];
  }
  if (/study|exam|padhai|test/.test(lower)) {
  ...
  if (/timer|alarm|reminder|yaad|minute|ghante/.test(lower)) {
    return ['⏰ 5 min timer', '⏰ 10 min timer', '⏰ 25 min Pomodoro'];
  }
  if (/target|goal|goal set|aim|lakshya/.test(lower)) {
    return ['🎯 Goals page kholo', '📊 Progress dekho', '📅 Deadline set karo'];
  }
  ...
  // flash mode: no generic chips - let topic detection handle it

  // No generic chips — cleaner chat
  return [];
}
