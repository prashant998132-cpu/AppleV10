...
export function generateFollowUps(userMessage, aiReply, mode = 'auto') {
  ...
  if (/neet|jee|medical|entrance/.test(u))
    return ['📚 Study plan do', '❓ MCQ practice', '🧬 Important topics', '📊 Progress check'];

  if (/biology|genetics|cell|photosynthesis|human body/.test(u))
    return ['🔬 Diagram banao', '❓ MCQ do', '📝 Short notes', '🧠 Mnemonics'];

  if (/physics|numerical|mechanics|optics|force/.test(u))
    return ['📐 Aur numericals', '📝 Formula sheet', '🔁 Concept phir explain', '⚡ Shortcut tricks'];

  if (/chemistry|organic|reaction|periodic/.test(u))
    return ['⚗️ Reactions list', '❓ MCQ practice', '📝 Quick notes', '🔁 Revision'];
