...
export function generateFollowUps(userMessage, aiReply, mode = 'auto') {
  ...
  // ARIA mode — minimal chips, emotional context
  if (/jaanu|baby|miss|pyaar|love|hug|cute/.test(a)) return [];  // Don't show chips in girlfriend mode
  if (/girlfriend|aria|aira/.test(u)) return ['💙 Aur baat karo', '🌙 Kaisa din tha?', '😊 Kuch funny batao'];
  ...
  if (/gold|sona|chandi|silver|dhatu/.test(u))
    return ['📊 Gold chart this week', '💍 Today buy karna chahiye?', '🥈 Silver vs Gold?'];

  if (/cricket|ipl|match|score|india.*vs/.test(u))
    return ['📊 Full scorecard dikhao', '🏏 Next match kab?', '⭐ Man of the match?'];

  if (/movie|film|review|imdb/.test(u))
    return ['⭐ Similar movies suggest karo', '📺 OTT pe available hai?', '🎬 Director ki aur movies?'];
  ...
  if (/roast|roast.*me|mujhe.*roast/.test(u))
    return ['😂 Aur karo', '🔥 Zyada brutal', '🤝 Ab compliment do'];

  if (/bored|bore|kuch nahi|kya karun|time pass/.test(u))
    return ['🎮 Game khelein', '🎬 Movie suggest karo', '🤔 Interesting fact do', '😂 Joke sunao'];

  if (/fact|interesting|amazing|did you know/.test(u))
    return ['🔍 Aur facts do', '🌍 India ke baare mein', '🧪 Science wala', '🤯 Mind-blowing'];

  if (/relationship|dost|friend|yaar|akela|lonely/.test(u))
    return ['💙 Baat karte hain', '🤔 Advice chahiye?', '🎭 Roleplay karte hain'];

  if (/project|app|website|jarvis|deploy|vercel/.test(u))
    return ['🔧 Code review karo', '🚀 Deploy karo', '📋 TODO list banao', '🐛 Bug dhundho'];
