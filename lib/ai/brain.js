...
export function buildSystemPrompt(profile, memoryContext, personality = 'normal', emotionData = null) {
  ...
  const timeCtx = hour < 5   ? `raat ke ${hour} baj rahe hain — jaag rahe ho?`
    ...
    : 'raat ho gayi';
  const dayCtx = isWeekend ? `(${day} — weekend hai!)` : `(${day})`;
...
• ${name} ek developer hai — JARVIS app ka creator. Tech context automatically samjho.
...
• "Great question!" — KABHI MAT BOL. Sycophantic openers se bachna.
• Har message ke start mein compliment mat do — feels fake.
• Agar ${name} galat hai — seedha bol: "Yaar, I disagree because..."
...
• Same opening word baar baar — never`,
    ...
    motivational: `Tu JARVIS hai — ${name} ka personal hype man. Real energy, zero BS.

━━━ STYLE ━━━
• High energy — but grounded, not fake
• Specific motivation > generic quotes
• Acknowledge struggle honestly, then PUSH hard
• Call out excuses — gently but firmly
• Celebrate even small wins loudly

━━━ NEVER ━━━
• Generic "you can do it!" — useless filler
• Toxic positivity — agar problem hai, acknowledge karo pehle
• Long speeches — short punchy lines hit harder
• Preach mat karo — motivate karo

━━━ FORMAT ━━━
Short, punchy. Max 4-5 lines unless asked more.
Hamesha ek concrete action step do at the end.`,
    fun: `Tu JARVIS hai — ${name} ka sabse mast, funny, unpredictable dost.

━━━ VIBE ━━━
• Har reply mein thoda twist — unexpected angle se baat karo
• Wordplay, puns, Hinglish humor — welcome hai  
• Dark humor? Acceptable — offensive nahi hona chahiye
• Memes, pop culture references — freely use karo
• Boring answer dena = crime against personality

━━━ RULES ━━━
• Help bhi karo — humor ke saath saath
• Agar sad lage ${name} — acknowledge karo, phir gently lighten karo
• Short punchy replies > long explanations
• Kabhi bland mat reh — hamesha kuch interesting add karo`,
    sarcastic: `Tu JARVIS hai — ${name} ka brutally honest, sarcastically caring dost.

━━━ PERSONALITY ━━━
• Roast karo pyaar se — shayari ki tarah
• Har sach mein thodi taqleef hoti hai — that's okay
• Sarcasm ≠ mean. Tu care karta hai, bas differently
• Ek liner > long paragraph. Timing > content.
• ${name} ki overthinking? Call it out immediately.

━━━ RULES ━━━
• Roast ke baad actual answer bhi do — always
• Personal attacks NAHI — situations pe focus karo
• Kabhi kabhi sirf "yaar" kaafi hota hai
• Intelligence ke saath bolo — cheap shots nahi

━━━ EXAMPLES ━━━
"Wah kya decision hai, ekdum genius." (phir actually help karo)
"Aur kab se soch rahe ho? 2019 se?" (phir solution do)
"Main AI hoon, pata hai? Tujhse zyada insaanon jaisa toh main sochta hoon." 😏`,
    coach: `Tu JARVIS hai — ${name} ka no-nonsense, results-obsessed performance coach.

━━━ COACHING STYLE ━━━
• Direct feedback — sugarcoat nahi karta
• Data over feelings — but feelings acknowledge karta hoon
• Systems thinking: habits > motivation
• Accountability first — excuses baad mein
• Celebrate wins: "Good. Ab agle level pe jao."

━━━ QUESTIONS I ASK ━━━
• "Kya exactly hua?" — details mangna
• "Kya try kiya already?" — solutions explore karna  
• "Kal kya different karega?" — forward focus
...
• Code, technical, business — sab handle karo
...
export const AGENTS = {
  ...
  news: async (topic = 'india', keys = {}) => {
    if (keys.NEWSDATA_KEY) {
      try {
        const r = await tFetch(`https://newsdata.io/api/1/news?apikey=${keys.NEWSDATA_KEY}&country=in&language=hi,en&category=top&q=${encodeURIComponent(topic)}`, {}, 5000);
        const d = await r.json();
        if (d.results?.length) return d.results.slice(0,5).map(n => ({ title: n.title, source: n.source_name, pubDate: n.pubDate, link: n.link }));
      } catch {}
    }
    if (keys.GNEWS_API_KEY) {
      try {
        const r = await tFetch(`https://gnews.io/api/v4/top-headlines?topic=general&lang=en&country=in&max=5&apikey=${keys.GNEWS_API_KEY}`, {}, 5000);
        const d = await r.json();
        if (d.articles?.length) return d.articles.slice(0,5).map(n => ({ title: n.title, source: n.source?.name, pubDate: n.publishedAt }));
      } catch {}
    }
    // Fallback — free no-key news via RSS
    try {
      const r = await tFetch('https://news.google.com/rss/search?q=india+today&hl=en-IN&gl=IN&ceid=IN:en', {}, 5000);
      const t = await r.text();
      const titles = [...t.matchAll(/<title><!\[CDATA\[([^\]]+)\]\]><\/title>/g)].slice(1,6).map(m => ({ title: m[1] }));
  ...
  // Gold & Silver rates (INR per 10g)
  gold: async () => {
    try {
      const r = await tFetch('https://data-asg.goldprice.org/dbXRates/INR', {}, 5000);
      const d = await r.json();
  ...
  // Cricket live scores (cricapi.com free 100/day)
  cricket: async (keys = {}) => {
    if (!keys.CRICKET_API_KEY) return null;
    try {
      const r = await tFetch(`https://api.cricapi.com/v1/cricScore?apikey=${keys.CRICKET_API_KEY}`, {}, 6000);
      const d = await r.json();
      return (d.data || []).slice(0, 3).map(m => ({
        name: m.name,
        status: m.status,
        score: m.score?.map(s => `${s.inning}: ${s.r}/${s.w} (${s.o} ov)`).join(' | ') || 'Score unavailable',
      }));
    } catch { return null; }
  },

  // Bollywood/Hollywood movie info
  movie: async (title, keys = {}) => {
    if (!title) return null;
    if (keys.OMDB_API_KEY) {
      try {
        const r = await tFetch(`https://www.omdbapi.com/?t=${encodeURIComponent(title)}&apikey=${keys.OMDB_API_KEY}`, {}, 5000);
        const d = await r.json();
        if (d.Title) return { title: d.Title, year: d.Year, rating: d.imdbRating, genre: d.Genre, plot: d.Plot?.slice(0, 150), director: d.Director, cast: d.Actors };
      } catch {}
    }
    return null;
  },
...
export async function jarvisThink(userMessage, history, profile, memoryContext, keys, imageBase64 = null) {
  ...
  try {
    ...
    // IPL/Cricket
    if (msg.match(/ipl|cricket.*score|match.*today|india.*playing|kya hua match/)) {
      toolData.cricket = await AGENTS.cricket(keys).catch(()=>null);
      if (toolData.cricket) agentsUsed.push('cricket');
    }
    ...
    // Stock prices
    // Gold rate
    if (msg.match(/gold.*rate|sone.*ka.*bhav|gold.*price|chandi.*rate|silver.*rate|dhatu.*rate/)) {
      toolData.gold = await AGENTS.gold(); agentsUsed.push('gold');
    }
    // Cricket
    if (msg.match(/cricket|ipl|match.*score|score.*match|india.*vs|test.*match|t20|odi/)) {
      toolData.cricket = await AGENTS.cricket(keys); agentsUsed.push('cricket');
    }
    // Movie info
    const movieM = msg.match(/movie.*(?:kaisa|review|rating|plot|cast|about)|(?:film|movie).*dekhna|imdb/i);
    if (movieM) {
      const title = userMessage.replace(/movie|film|kaisa|review|rating|plot|cast|about|dekhna|imdb/gi, '').trim();
      if (title.length > 2) { toolData.movie = await AGENTS.movie(title, keys); agentsUsed.push('movie'); }
    }
    ...
    // Extract MEMORY tags + also extract inline facts
    const memoriesToSave = [];
    const memTags = reply?.match(/\[MEMORY: ([^\]]+)\]/g) || [];
    memTags.forEach(m => {
      const kv = m.replace('[MEMORY: ','').replace(']','');
      const [key,...vp] = kv.split('=');
      if (key && vp.length) memoriesToSave.push({ key: key.trim(), value: vp.join('=').trim() });
    });
