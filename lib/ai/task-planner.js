  ...
  for (let i = 0; i < workflow.steps.length; i++) {
    ...
    try {
      ...
      switch (step.tool) {
        ...
        case 'weather':
          if (agents?.weather) {
            const savedLoc = typeof localStorage !== 'undefined' ? JSON.parse(localStorage.getItem('jarvis_user_location') || '{}') : {}; const cityName = savedLoc.city || profile?.city || 'Delhi'; const w = await agents.weather(cityName);
