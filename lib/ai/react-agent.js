...
const TOOL_REGISTRY = {
  ...
  gold_rate: {
    desc: 'Get current gold and silver rate in India (INR per 10g). Input: none needed',
    fn: async () => {
      try {
        const r = await fetch('https://data-asg.goldprice.org/dbXRates/INR', { signal: AbortSignal.timeout(5000) });
        const d = await r.json();
        const g = Math.round(d.items?.[0]?.xauPrice / 31.1035 * 10);
        const s = Math.round(d.items?.[0]?.xagPrice / 31.1035 * 10);
        return `Gold: ₹${g}/10g | Silver: ₹${s}/10g`;
      } catch { return 'Gold rate unavailable right now'; }
    },
  },
  web_search: {
  ...
  // NEW: Read any URL
  read_url: {
    desc: 'Read and summarize any webpage. Input: full URL (https://...)',
    fn: async (input) => {
      try {
        if (!input?.startsWith('http')) return 'Please provide a full URL starting with https://';
        const r = await fetch(`https://r.jina.ai/${input}`, {
          headers: { 'Accept': 'text/plain' },
          signal: AbortSignal.timeout(12000),
        });
        if (!r.ok) return `Could not read URL: ${r.status}`;
        const text = await r.text();
        return `Content from ${input}:
${text.slice(0, 2500)}`;
      } catch (e) { return `URL read failed: ${e.message}`; }
    },
  },
  // NEW: Jina web search (free, no key)
  search_web: {
    desc: 'Search the web for current information. Input: search query',
    fn: async (input) => {
      try {
        const r = await fetch(`https://s.jina.ai/${encodeURIComponent(input || '')}`, {
          headers: { 'Accept': 'text/plain' },
