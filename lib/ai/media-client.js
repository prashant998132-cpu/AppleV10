...
// ─── DEEZER FREE MUSIC SEARCH (no key, no auth) ──────────────
export async function searchMusicFree(query) {
  try {
    // Deezer — completely free, no key, returns previews (30s MP3)
    const r = await fetch(`https://api.deezer.com/search?q=${encodeURIComponent(query)}&limit=5&output=json`);
    if (!r.ok) throw new Error('Deezer error');
    const d = await r.json();
    return (d.data || []).map(t => ({
      title: t.title,
      artist: t.artist?.name,
      preview: t.preview,  // 30s free MP3 preview
      cover: t.album?.cover_medium,
      url: t.link,
      source: 'Deezer',
    }));
  } catch {
    // Fallback: Jamendo (free license music)
    try {
      const r = await fetch(`https://api.jamendo.com/v3.0/tracks/?client_id=b6747d04&format=json&limit=5&search=${encodeURIComponent(query)}&audioformat=mp32`);
      const d = await r.json();
      return (d.results || []).map(t => ({
        ...
        cover: t.image,
        url: t.shareurl,
        source: 'Jamendo (free license)',
      }));
    } catch { return []; }
  }
}

// ─── MOVIE INFO (TMDB — free key signup required) ────────────
export async function searchMovie(query, tmdbKey) {
  if (!tmdbKey) {
    // No-key fallback: use TheAudioDB for basic info
    try {
      const r = await fetch(`https://theaudiodb.com/api/v1/json/2/search.php?s=${encodeURIComponent(query)}`);
      const d = await r.json();
      return d;
    } catch { return null; }
  }
  try {
    const r = await fetch(`https://api.themoviedb.org/3/search/movie?api_key=${tmdbKey}&query=${encodeURIComponent(query)}&language=en-US&include_adult=false`);
    const d = await r.json();
    return (d.results || []).slice(0,5).map(m => ({
      title: m.title,
...
export function pollinationsUrl(prompt, width = 512, height = 512, seed = Date.now()) {
  return `https://image.pollinations.ai/prompt/${encodeURIComponent(prompt)}?width=${width}&height=${height}&nologo=true&seed=${seed}&enhance=true&model=flux&safe=false`;
}
...
export async function speakWithEmotion(text, opts = {}) {
  ...
  // Puter.js FREE unlimited TTS (no key needed — Amazon Polly quality!)
  if (!sarvamKey) {
    try {
      if (typeof window !== 'undefined' && !window.__puterTtsLoaded) {
        // Lazy load Puter.js
        await new Promise((res, rej) => {
          if (window.puter) { res(); return; }
          const s = document.createElement('script');
          s.src = 'https://js.puter.com/v2/';
          s.onload = () => { window.__puterTtsLoaded = true; res(); };
          s.onerror = rej;
          document.head.appendChild(s);
        });
      }
      if (window.puter?.ai?.txt2speech) {
        const cleanText2 = text.replace(/[*_#[\]`~]/g, ' ').trim().slice(0, 600);
        const audio = await window.puter.ai.txt2speech(cleanText2);
        if (audio) {
          audio.onended = onEnd || null;
          audio.play();
          return { provider: 'puter-tts', emotion };
        }
      }
