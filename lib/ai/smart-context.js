...
export function getTimeContext() {
  ...
  if (isWeekend && h >= 9 && h < 18) return {
    context: 'weekend',
    emoji: '🎉',
    greeting: 'Weekend vibes!',
    suggestions: [
      { icon: '🎬', text: 'Movie dekhna hai', cmd: 'koi achhi movie recommend karo weekend ke liye' },
      { icon: '🍕', text: 'Khaana order karo', cmd: 'Zomato kholo' },
      { icon: '🎵', text: 'Chill playlist', cmd: 'chill playlist recommend karo' },
      { icon: '💪', text: 'Workout karo', cmd: 'weekend workout routine do' },
    ],
  };
  ...
  if (h >= 21 && h < 24) return {
    ...
    suggestions: [
      { icon: '📖', text: 'Din ka review karo', cmd: 'Aaj ka din review karo — kya achha hua, kya improve karna hai' },
      { icon: '🎯', text: 'Kal ki planning', cmd: 'Kal ke liye ek solid plan banao' },
      { icon: '🌙', text: 'Random fact sunao', cmd: 'Koi fascinating science/history fact batao' },
      { icon: '😴', text: 'Sleep tips', cmd: 'Better neend ke liye tips do — scientifically proven wale' },
    ],
  };
