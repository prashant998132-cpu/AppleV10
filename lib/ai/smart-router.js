...
export const PROVIDERS = {
  ...
  // ══ Gemini Flash-Lite — 1000 RPD free, ultra-fast ══
  gemini_flash_lite: {
    ...
    hinglish: true,
    streaming: true,
    bestFor: ['flash', 'simple', 'greetings'],
    costPerToken: 0,
  },
  ...
  gemini_flash: {
    ...
    model: 'gemini-2.5-flash',  // stable — preview-04-17 deprecated
    authKey: 'GEMINI_API_KEY',
    dailyLimit: 50,         // Google cut free tier 92% Dec 2025 — treat as limited
  ...
  gemini_1_5: {
    ...
    model: 'gemini-1.5-flash',  // fallback when 2.5 quota exhausted
    authKey: 'GEMINI_API_KEY',
    dailyLimit: 1500,
    speed: 'fast',
    quality: 'very_good',
  ...
  // ══ Groq Kimi K2 — 1T param MoE, best quality on Groq, 256K context ══
  groq_kimi_k2: {
    name: 'Groq Kimi K2',
    tier: 1,
  ...
  // ══ Groq GPT-OSS 20B — lighter, faster fallback ══
  groq_gpt_oss_20b: {
  ...
  // ══ Groq GPT-OSS 120B — replaced Maverick Feb 2026 ══
  groq_gpt_oss: {
  ...
  // ══ Groq Qwen 3 32B — replaced qwq-32b, excellent reasoning ══
  groq_qwen3: {
  ...
  groq_llama4_maverick: {
    name: 'Groq GPT-OSS 120B (Maverick slot)',
    tier: 1,
    url: 'https://api.groq.com/openai/v1/chat/completions',
    model: 'openai/gpt-oss-120b',  // llama-4-maverick DEPRECATED Feb 2026 → gpt-oss-120b
    authKey: 'GROQ_API_KEY',
  ...
  groq_llama4_scout: {
    ...
    authKey: 'GROQ_API_KEY',
    // Llama 4 Scout — 10M context, best free model on Groq
  ...
  groq_deepseek_r1: {
    name: 'Groq Llama 3.3 70B (Reasoning)',
    tier: 1,
    url: 'https://api.groq.com/openai/v1/chat/completions',
    model: 'llama-3.3-70b-versatile',  // deepseek-r1 deprecated Sep 2025, replaced
    authKey: 'GROQ_API_KEY',
    dailyLimit: 6000,
    warningAt: 5000,
    speed: 'medium',
...
export function getProviderOrder(mode = 'auto', message = '', history = [], availableKeys = {}) {
  ...
  if (mode === 'think') {
    return [
      ...
      'gemini_flash',
      'openrouter_deepseek',
      'groq_kimi_k2',  // Kimi K2 — 1T param, best reasoning
      'groq_qwen3',  // qwen3-32b excellent reasoning
      'groq_gpt_oss',
      'cerebras',
      'pollinations_text',
      'offline',
  ...
  if (mode === 'flash') {
    return [
      'groq_llama4_scout',     // #1 Groq · Llama 4 Scout 17B ~1s fastest
      'together_llama4',       // #2 Together · Llama 4 Scout ~2s fallback
      'gemini_flash',          // #3 Gemini 2.5 Flash ~3s Google fallback
      'gemini_flash_lite',     // #3b Flash-Lite if flash full
      'pollinations_text',     // #4 No key, browser direct ~5s
      'cerebras',              // alt fast fallback
      'groq_kimi_k2',
      'groq_qwen3',
      'mistral',
      'offline',
  ...
  if (mode === 'deep') {
    return [
      'groq_kimi_k2',          // 1T param MoE — best quality
      'gemini_flash',
      'gemini_1_5',
  ...
  if (complexity === 'simple') {
    ...
    if (/haan|nahi|ok|hi|hello|thanks|bye|kya|batao|yaar/.test(lower)) {
      return [
        'gemini_flash_lite',   // 1000 RPD ultrafast for greetings
        'groq_llama4_scout',   // 460 t/s backup
        'cerebras',
        'pollinations_text',
        'mistral',
        'offline',
  ...
  // Normal — balanced (Groq first after Dec 2025 Gemini cuts)
  return [
    'groq_kimi_k2',
    'groq_llama4_scout',
    'cerebras',
...
async function withRetry(fn, maxRetries = 2, baseDelay = 800) {
  for (let i = 0; i <= maxRetries; i++) {
    try {
      ...
      const is429 = err?.message?.includes('429') || err?.message?.includes('rate') || err?.status === 429;
...
export async function callProvider(providerId, messages, systemPrompt, availableKeys, options = {}) {
  ...
  const res = await fetch(p.url.replace(':streamGenerateContent', ':generateContent').replace('alt=sse&', ''), {
    ...
    signal: _timeoutSignal(15000), // 15s timeout
  });
