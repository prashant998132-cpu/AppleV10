...
export function isPuterEnabled() {
  // Puter only loads if user explicitly enabled it OR if it's already loaded
  if (typeof window === 'undefined') return false;
  if (window.puter) return true; // already loaded
  try { return localStorage.getItem('jarvis_puter_enabled') === 'true'; } catch { return false; }
}

export function setPuterEnabled(val) {
  try { localStorage.setItem('jarvis_puter_enabled', val ? 'true' : 'false'); } catch {}
}
...
export async function loadPuter() {
  ...
  // Only load if user explicitly opted in
