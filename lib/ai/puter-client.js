...
export const PUTER_MODELS = {
  best:  ['claude-sonnet-4', 'google/gemini-2.5-flash', 'gpt-5.2', 'openrouter:deepseek/deepseek-chat-v3-0324:free'],
  fast:  ['gpt-5-nano', 'gpt-4.1-nano', 'google/gemini-2.5-flash'],
  image: ['gpt-image-1.5', 'dall-e-3'],
  search:['openai/gpt-5.2-chat', 'gpt-5.2', 'gpt-5-nano'],
};
...
export async function puterSearchChat(query, systemPrompt) {
  const puter = await loadPuter();
  if (!puter?.ai?.chat) return null;
  for (const m of PUTER_MODELS.search) {
    try {
      const response = await puter.ai.chat(
        [...(systemPrompt ? [{ role: 'system', content: systemPrompt }] : []), { role: 'user', content: query }],
        { model: m, tools: [{ type: 'web_search' }], stream: false }
      );
      const text = extractText(response);
      if (text) return { reply: text, model: 'puter-search', hasSearch: true };
    } catch {}
  }
  return null;
}
