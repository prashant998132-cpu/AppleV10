...
export function getKeys() {
  return {
    ...
    MOONSHOT_API_KEY:      process.env.MOONSHOT_API_KEY      || null,  // v11.3: Kimi K2.5 thinking model — platform.moonshot.ai (free trial)
    NVIDIA_API_KEY:         process.env.NVIDIA_API_KEY         || null,  // v11.1: NVIDIA NIM
    TMDB_API_KEY:           process.env.TMDB_API_KEY           || null,  // v11.2: The Movie Database — free signup tmdb.org
    OMDB_API_KEY:           process.env.OMDB_API_KEY           || null,  // v11.2: OMDb — 1000/day free omdbapi.com
    LASTFM_API_KEY:         process.env.LASTFM_API_KEY         || null,  // v11.2: Last.fm music data — free last.fm/api — Kimi K2.5, DeepSeek free ~40RPM
    ...
    // NEWSDATA_KEY already declared above — v10 dedup
    ...
    TODOIST_API_KEY:        process.env.TODOIST_API_KEY        || null,
    OPENWEATHER_KEY:        process.env.OPENWEATHER_KEY        || null,
    ...
    BITLY_TOKEN:            process.env.BITLY_TOKEN            || null,
    // ── Push Notifications ────────────────────────────────────
    VAPID_PUBLIC_KEY:       process.env.VAPID_PUBLIC_KEY       || null,
    VAPID_PRIVATE_KEY:      process.env.VAPID_PRIVATE_KEY      || null,
    VAPID_SUBJECT:          process.env.VAPID_SUBJECT          || 'mailto:jarvis@app.local',
    CRON_SECRET:            process.env.CRON_SECRET            || null,  // Vercel cron auth
    // PIXABAY_KEY already declared above — removed duplicate
  };
}

// ─── PROVIDER LIMITS (daily) ──────────────────────────────────────
export const DAILY_LIMITS = {
  gemini:      50,   // Dec 2025: Google cut free tier ~92% — Flash actual RPD ~50
  gemini_lite:  1000, // Gemini 2.5 Flash-Lite — 1000 RPD free, ultrafast
  groq:        6000,
  cerebras:    1000,
  sambanova:   999999,
  together:    9999,
